...
import/startup from a content hash of every precached static asset (see
`_compute_cache_version` in `backend/routers/pwa.py`) — it changes
automatically whenever a precached file's bytes change, with no manual
bump step. Icons are generated by `scripts/generate-icons` (Pillow, dev-only dep).
